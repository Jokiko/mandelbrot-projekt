...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item[b)] wenn Emulator noch nicht erstellt
			...
			\item[4.] klicke \enquote{next}
			\item[5.] wähle, falls vorhanden, aktuellstes Betriebssystem, ansonsten downloade es
			\item[6.] klicke \enquote{next}
			\item[7.] klicke \enquote{finish}
			\end{itemize}
		\item[c)] falls gewünscht kann mithilfe von \enquote{Run on Multiple Devices} die App auf mehreren Geräten gleichzeitig gestartet werden
			\begin{itemize}
		...
		\end{itemize}
	\end{itemize}
	\item starte App (App wird installiert und öffnet automatisch)
\end{enumerate}
\begin{figure}[htbp] 
	\centering
...
Dieses Problem liegt nicht daran, dass der readerThread nicht richtig beendet wird, wenn die App geschlossen wird, da ein Aufruf von \enquote{readerThread.interrupt()} nicht hilft.\par
