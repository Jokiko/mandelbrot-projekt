...
void receiveSize() {
	...
	if (image_width > 1024) {
		printf("Image Width greater than 1024! Can't use Cuda client.\n");
...
void getTask() {
	...
	if (strcmp(check, "task") == 0) {
		...
		task_y = *reinterpret_cast<int*>(y_bytes);
		task_xMove = *reinterpret_cast<double*>(xMove_bytes);
		task_yMove = *reinterpret_cast<double*>(yMove_bytes);
		task_zoom = *reinterpret_cast<double*>(zoom_bytes);
		task_itr = *reinterpret_cast<int*>(itr_bytes);

		cudaEvent_t start, stop;
		cudaEventCreate(&start);
		cudaEventCreate(&stop);
		cudaEventRecord(start);
		calculate << <1, image_width >> > (image_width, image_height, task_y, task_xMove, task_yMove, task_zoom, task_itr, device_result_package);
		...
		printf("Time per task calculation: %f milliseconds \n", milliseconds);
		copyResults();
